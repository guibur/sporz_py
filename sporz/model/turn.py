...
class ChiefElectionTurn(AbstractTurn):
    ...
    def get_info(self):
        return {'type': D_CHIEF_ELECTION, 'elected': self.chief}
...
class MutantTurn(AbstractTurn):
    ...
    def act(self, players, state):
        result_actions = {'type': Turn.N_MUTANTS, 'paralyzed': None, 'mutated': None, 'failed_to_mutate': None, 'killed': None}
        state['paralyzed'] = self.paralyze
        result_actions['paralyzed'] = self.paralyze
        if self.do == 'kill':
            state['dead'].add(self.target)
            result_actions['killed'] = self.target
        elif self.do == 'mutate':
            if players[self.target]['genome'] is not Genome.RESISTANT:
                state['mutants'].add(self.target)
                result_actions['mutated'] = self.target
            else:
                result_actions['failed_to_mutate'] = self.target
        else:
...
class ComputerScientist(AbstractTurn):
...
def create_turn(turn_type, game, **kwargs):
    TURN_CLASSES = {
        ...
        Turn.N_COMPUTER_SCIENTIST: ComputerScientist,
