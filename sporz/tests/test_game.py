...
def test_first_chief_election():
    ...
    assert game.get_next_turn() == Turn.D_CHIEF_ELECTION
    assert game.get_next_turn() == Turn.D_CHIEF_ELECTION
    assert game.get_next_turn() == Turn.D_CHIEF_ELECTION
    game.act(chief=1)
...
def test_state_sequence():
    game = Game(PLAYER_LIST, ROLE_LIST, GENOME_LIST)
    game.act(chief=1)
    assert game.get_next_turn() is Turn.N_MUTANTS
    ...
    assert game.get_next_turn() is Turn.N_DOCTORS
    assert game.get_prepare_action_info(Turn.N_DOCTORS)["players"] == [1]
